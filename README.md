...
2. Run medGAN using the ".matrix" file generated by process_mimic.py.  
The command is `python medgan.py <matrix file> <output path> --data_type=["binary", "count"]`.

3. After the training, if you want to generate synthetic records, please see the source code of "medgan.py". 
There is a commented block at the bottom of the source code, which will let you generate synthetic records.
